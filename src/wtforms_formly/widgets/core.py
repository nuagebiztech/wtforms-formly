    ...
    def __call__(self, field, **kwargs):
        ...
        for subfield in field:
            if subfield.type in ("HiddenField", "CSRFTokenField"):
                ...
                html.append(
                    f"<tr><th>{str(subfield.label)}</th><td>{hidden}{str(subfield)}</td></tr>"
                )
                hidden = ""
        if self.with_table_tag:
            html.append("</table>")
...
class Input:
    ...
    def __call__(self, field, **kwargs):
        ...
        return Markup(f"<input {self.html_params(name=field.name, **kwargs)}>")
    ...
    def __call__(self, field, **kwargs):
        ...
        if field.has_groups():
            for group, choices in field.iter_groups():
                html.append(f"<optgroup {html_params(label=group)}>")
                for choice in choices:
                    if len(choice) == 4:
                        ...
                        warnings.warn(
                            ...
                            "wtforms_formly 3.1, this will be mandatory in wtforms_formly 3.2",
                            DeprecationWarning,
                            stacklevel=2,
                        )
