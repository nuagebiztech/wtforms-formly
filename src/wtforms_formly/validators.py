...
class Length:
    ...
    def __call__(self, form, field):
        ...
        raise ValidationError(
            message % {"min": self.min, "max": self.max, "length": length}
        )
...
class NumberRange:
    ...
    def __call__(self, form, field):
        ...
        raise ValidationError(message % {"min": self.min, "max": self.max})
...
class Email:
    ...
    installed. For ex: pip install wtforms_formly[email].
...
class AnyOf:
    ...
    def __call__(self, form, field):
        ...
        raise ValidationError(message % {"values": self.values_formatter(self.values)})
