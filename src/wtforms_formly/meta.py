from wtforms_formly import i18n
from wtforms_formly.utils import WebobInputWrapper
from wtforms_formly.widgets.core import clean_key
...
class DefaultMeta:
    ...
    def render_json(self, field, render_kw) -> dict:
        """
        render_field allows customization of how widget rendering is done.
        ...
        if other_kw is not None:
            other_kw = {clean_key(k): v for k, v in other_kw.items()}
            render_kw = dict(other_kw, **render_kw)
        return field.widget.__json__(field, **render_kw)
    ...
    def build_csrf(self, form):
        ...
        :class:`wtforms_formly.csrf.session.SessionCSRF`.
